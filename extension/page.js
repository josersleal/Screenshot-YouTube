...
function CaptureScreenshot() {
  ...
  function SetTitle() {
    if (headerEls.length > 0) {
      ...
      // console.log('Title@SetTitle:' + title);
      return true;
    } else {
      return false;
    }
  ...
  // console.log('After Title@SetTitle:' + title);
  
	function formatTime(seconds) {
		// Remove dots from the input and parse it as an integer
		var parsedSeconds = parseInt(seconds.toString().replace(/\./g, ''));
		
		var hours = Math.floor(parsedSeconds / 3600);
		var remainingSeconds = parsedSeconds % 3600;
